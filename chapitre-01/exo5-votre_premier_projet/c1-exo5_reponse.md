...
Le fichier `Applications/MonEssai/MonEssai.jenga` contient :
...
## 3. Déclaration dans le workspace
...
## 4. Vérification avec `jenga info`

J’ai vérifié que le projet apparaît bien dans le workspace avec :
...
Résultat :
``MonEssai`` est present a la derniere ligne des projets:
...
Le projet `MonEssai` apparaît donc bien dans le workspace et est reconnu comme une `ConsoleApp`.
...
Le projet `MonEssai` est dans le workspace.

`jenga info` le reconnaît comme une `ConsoleApp`, et `jenga build --project MonEssai` réussit avec un temps total de **0,70 s**.

L’exécutable généré au final est :

```
Build\Bin\Debug-Windows\MonEssai\MonEssai.exe
```
